...
#загрузить эмбы
chroma_db = Chroma(persist_directory='./chromadb_full', collection_name='the_world', embedding_function=emb)
...
for ds_num in range(1,5):
    ...
    for k_c in range(1, K_c+1):
        ...
        qds['ret_chunks'] = qds["question"].apply(lambda x: list(elem.metadata['id'] for elem in ensemble_retriever.invoke(x, k=k_c+1)[:k_c])) 
...
res_df.to_excel(f'results/new/kc_ans_alt.xlsx', index=False)
